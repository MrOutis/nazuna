...
try {
    ...
    if (!config.prefixo || !config.nomebot || !config.numerodono, !config.apikey_vex) {
        throw new Error('Configuração inválida: campos obrigatórios ausentes (prefixo, nomebot, numerodono, apikey_vex)');
    }
...
async function createBotSocket(authDir) {
    try {
        ...
        // mudamos para a versão pré definida 
        const version = config["WA_WEB_VERSION"].value;
        ...
        NazunaSock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                console.log(`✅ Bot ${nomebot} iniciado com sucesso! Prefixo: ${prefixo} | Dono: ${nomedono} | Api-key: ${apikey_vex}`);
